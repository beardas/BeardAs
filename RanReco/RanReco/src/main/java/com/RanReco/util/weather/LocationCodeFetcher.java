...
public class LocationCodeFetcher {
    ...
    public Coord fetchLocationCode(String[] saLocation) { // 인자: {시도, 시군구, 동면읍}
    ...
    public String fetchLocationSidoCode(String[] saLocation) { // 인자: {시도, 시군구, 동면읍}
        JSONArray jsonArrTop = null;

        jsonArrTop = jjp.getRemoteJSONArray(getStrUrl("top"));
        mapTop = jjp.getJsonSubMap(jsonArrTop);
        
        return mapTop.get(saLocation[0]);
    }
